 ...
 *
 */
public class UFS2_Unbalanced implements Serializable {
    ...
    public static void main(String[] args) {
        ...
        // Uniform distribution of trees across hosts, with the last host not getting
        // anything
        WORLD.broadcastFlat(() -> {
            int destination = 0;
            final CollectiveMoveManager mm = new CollectiveMoveManager(WORLD);
            for (final LongRange lr : trees.getAllRanges()) {
                trees.moveRangeAtSync(lr, place(destination % (WORLD.size() - 1)), mm);
                destination++;
            }
            mm.sync();
        });
    ...
    public static void printUsage() {
        System.err.println("Udage: mpirun -np <N> java -cp <classpath> " + Launcher.class.getCanonicalName() + " "
                + UFS2_Unbalanced.class.getCanonicalName() + " ARGUMENTS");
