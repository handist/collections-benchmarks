...
public class MoldynHybrid implements Serializable {
	...
	public void runiters0(boolean isWarmup) throws MPIException {
		...
		myAccM = new MyAccumM(oneX);
			...
			for (move = 0; move < movemx; move++) {
				...
				List<List<RangedListProduct<Particle, Particle>>> prodsX = new RangedListProduct<>(one, one, true)
						.splitNM(Ndivide, Ndivide, placeGroup.rank(), placeGroup.size(), Nworkers, new java.util.Random(0));
				List<LocalStatics> lss = new ArrayList<>();
				finish(()-> {
					for (List<RangedListProduct<Particle, Particle>> prods : prodsX) {
						LocalStatics ls = new LocalStatics();
						lss.add(ls);
						async(() -> {
							for (RangedListProduct<Particle, Particle> prod : prods) {
				...
				oneX.allreduce((p)-> new Sp(p.xforce,p.yforce,p.zforce), (p,sp)->{ p.xforce +=sp.x; p.yforce +=sp.y; p.zforce+=sp.z;});
				for(LocalStatics ls: lss) {
					epot += ls.epot;
					vir += ls.vir;
	...
	private void force1(Particle p0, RangedList<Particle> pairs, double side, double rcoff, LocalStatics s) {
		...
		RangedList<Sp> fs = myAccM.getCopy(pairs.getRange());
		Iterator<Sp> fiterator = fs!=null? fs.iterator(): null;
		double vir0 = 0.0, epot0 = 0.0;
		int inters0 = 0;
		for(Particle p1: pairs){
			Sp p1f = fiterator.next();
