...
do
	...
	touch ${OUTFILE}
	touch ${ERRFILE}
	echo "[Run ${run}] `pjsub -N ${BENCHMARK_NAME}${run} -X --rsc-list rscgrp=${RSCGRP},node=${NB_HOSTS},elapse=${TIMEOUT} -o ${OUTFILE} -e ${ERRFILE} ${BENCH_HOME}/launchers/${HOST_TYPE}/program-nompi.sh 2>&1`"
    fi
done
