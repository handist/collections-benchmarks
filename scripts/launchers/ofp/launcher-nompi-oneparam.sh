...
do
    ...
    mkdir -p param${value}
    export EXTRA_ARGS=param${value}/${CONFIG_NAME}_${BENCHMARK_NAME}_${PARAM}${value}_Run${run}
	...
	touch ${OUTFILE}
	touch ${ERRFILE}
	echo "[${PARAM}:${value} Run ${run}] `pjsub -N ${BENCHMARK_NAME}${run} -X --rsc-list rscgrp=${RSCGRP},node=${NB_HOSTS},elapse=${TIMEOUT} -o ${OUTFILE} -e ${ERRFILE} ${BENCH_HOME}/launchers/${HOST_TYPE}/program-nompi.sh 2>&1`"
    fi
done
done
STAMP=`date +"%m-%d-%y %R"`
